...
#define DEFINE_ENUMOP(TYPE)                              \
    ...
    struct BasicBlock
    {
        ...
        BasicBlock(bool block = true, bool array = true, bool bracket = true) : c_block(block), arr_block(array), bracket(bracket) {}
    };
    ...
    bool cmdline_read(Reader<Buf> *self, Ret &ret, int *&res, bool begin)
    {
        ...
        {
            auto prev = ret.size();
            self->readwhile(ret, untilincondition, [](char c)
                            { return c != ' ' && c != '\t'; });
            if (ret.size() == prev)
            {
                *res = 0;
